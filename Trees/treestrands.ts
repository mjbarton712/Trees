export function chooseStrandGeneric(rand:number):string{
    let nextYear:string = "";
...
export function chooseStrandFunky(rand:number){
    //not doing anything with rand yet^
    let nextYear:string = "F";
    let branches:string [] = ["[LX[V]]", "[RX[V]]", "[&X[V]]", "[^X[V]]", "[L^X[V]]", "[L&X[V]]", "[R&X[V]]", "[R^X[V]]",
...
export function chooseStrandCurvy(rand:number){
    let nextYear:string = "";
